...
export const alt = invitation.meta.title;

const { couple, date, venue } = invitation;

/** How the link looks when it lands in WhatsApp or iMessage. */
export default async function OpengraphImage() {
  // Bundled rather than fetched, so a build never depends on Google Fonts.
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        />
        {/* After the gradient, so the hairline is not washed over by it. */}
        <div
          style={{
            position: "absolute",
            inset: 28,
        ...
        {/* One text child on purpose: Satori requires an explicit display on
